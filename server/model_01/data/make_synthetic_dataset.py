...
def _add_upsampling_artifact(img: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    ...
    leaves in real generator output. Amplitude is kept small (not obvious at a
    glance) since real AIGC artifacts are typically subtle too."""
    size = img.shape[0]
...
def generate_dataset(out_dir: Path, n_per_class: int, size: int, seed: int) -> None:
    ...
    for i in range(n_per_class):
        ...
        fake_channel = _add_upsampling_artifact(base.copy(), rng)
...
def main():
    ...
    generate_dataset(out_dir, args.n_per_class, args.size, args.seed)
